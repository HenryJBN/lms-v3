...
class CourseService {
  ...
  async getCategories(): Promise<CourseCategory[]> {
    try {
      return await apiClient.get<CourseCategory[]>(
        API_ENDPOINTS.courseCategories
      );
    } catch (error) {
  ...
  async updateCourse(courseId: string, data: CourseUpdate): Promise<Course> {
    try {
      ...
      if (isFileUpload) {
        ...
        return await apiClient.put<Course>(
  ...
  async publishCourse(courseId: string): Promise<Course> {
    try {
      return await apiClient.post<Course>(
        `${API_ENDPOINTS.courses}/${courseId}/publish`,
        {}
      );
    } catch (error) {
