...
class ProgressService {
  async getLessonProgress(lessonId: string, enrollmentId: string): Promise<LessonProgress> {
    try {
  ...
  async updateLessonProgress(data: UpdateLessonProgressRequest): Promise<LessonProgress> {
    try {
      return await apiClient.post<LessonProgress>(API_ENDPOINTS.lessonProgress, data)
    } catch (error) {
